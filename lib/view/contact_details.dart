  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<ContactBloc, ContactState>(builder: (context, state) {
      if (state is ContactIsLoadedState) {
        print("oftad");
        return ContactDetailsView(context, state.getContact.phone, state.getContact.email, state.getContact.name, state.getContact.lastName, state.getContact.imgPath);
      }
      return Center(child: Text("ops error"),);
    });
  }

  Widget ContactDetailsView(context, phone, String email, String name, String lastname, String img) {
    return WillPopScope(
      onWillPop: () {
        Navigator.push(
            context,
                ...
                child: CircleAvatar(
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                          borderRadius: BorderRadius.circular(50),
                          image: DecorationImage(
                            image:  Image.file(File(img)).image,fit: BoxFit.fill)
                      ),
                    )
                  // Image.file(_image),
                ),
