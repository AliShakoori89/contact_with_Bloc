...
class DatabaseHelper{
  ...
  Future<bool> saveContact (Contact contact) async{
    ...
    await myContact.insert(Contact.TABLENAME, contact.toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace);
    print('aaaaaaaaaaaaaaaaaaaaaaa${contact.toMap()}');
    return true;
  }

  Future<List> getAllContacts() async {
    var myContact = await database;
    List listMap = await myContact.rawQuery('SELECT * FROM my_table');
  ...
  //   var myContact = await database;
  //   var result = await myContact.query("my_table", where: "id = ", whereArgs: [id]);
  //   return result.isNotEmpty ? TABLENAME.fromMap(result.first) : Null;
  //
  // }
  ...
  deleteContact(int id) async {
    print('deleteContact in database');
    var myContact = await database;
    var result = myContact.delete('my_table' , where: "$columnId=$id");
    return result;
  }

  updateContact(Contact person) async {
    var myContact = await database;
    await myContact.update(Contact.TABLENAME,
        person.toMap(),
